...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(calc_eval(operator), exp.rest)
...
def floor_div(args):
    ...
    20
    """
    def calc_div(diviend, divisors):
        if divisors == nil:
            return calc_eval(diviend)
        else:
            return calc_div(diviend, divisors.rest) // calc_eval(divisors.first)
    return calc_div(args.first, args.rest)
...
def eval_and(expressions):
    ...
    True
    """
    def calc_and(expressions):
        if expressions == nil:
            return scheme_t
        result = calc_eval(expressions.first)
        if (type(result) == bool and result == scheme_f) or expressions.rest == nil:
            return result
        else:
            return calc_and(expressions.rest)
    return calc_and(expressions)
