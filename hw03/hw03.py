...
def interleaved_sum(n, odd_term, even_term):
    ...
    def helper(n, odd_term, even_term):
        ...
        if n == 0:
            ...
            ret = helper(n - 1, odd_term, even_term)
            if toggle == 1:
                toggle = 0
                return ret + odd_term(n)
            else:
                toggle = 1
                return ret + even_term(n)
...
def count_coins(total):
    ...
    True
    """
    def try_smaller(total, coin):
        if coin == 1:
            return dfs(total - coin, coin)
        else:
            return dfs(total - coin, coin) + try_smaller(total, next_smaller_coin(coin))
    def dfs(total, coin):
        if total < 0:
            return 0
        elif total == 0:
            ...
            return try_smaller(total, coin)
    return try_smaller(total, 25)
