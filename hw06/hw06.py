...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            if self.balance > 0:
                msg = f'Here is your {self.name} and ${self.balance} change.'
                self.balance = 0
            else:
    ...
    def add_funds(self, funds):
        if self.stock == 0:
            msg = f'Nothing left to vend. Please restock. Here is your ${funds}.'
        else:
            self.balance += funds
...
def two_list(vals, counts):
    ...
    for i in range(n):
        rev = n - i - 1
        for _ in range(counts[rev]):
            if ret == None:
                ret = Link(vals[rev])
            else:
                ret = Link(vals[rev], ret)
    return ret
