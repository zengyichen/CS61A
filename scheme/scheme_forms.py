...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # expressions = (signature value) in scheme
        #             = Pair(signature, Pair(value, nil)) in Python
        # therefore, value = expressions.rest.first
        return signature
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        lambda_form = Pair('lambda', Pair(signature.rest, expressions.rest))
        scheme_eval(Pair('define', Pair(signature.first, Pair(lambda_form, nil))), env)
