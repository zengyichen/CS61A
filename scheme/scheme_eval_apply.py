...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        args_python = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operator, args_python, env)
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        # BEGIN PROBLEM 2
        # procedure(args, env)
        args_python, args_scheme = [], args
        while args_scheme != nil:
        ...
        try:
            ...
            if procedure.need_env:
                return procedure.py_func(*args_python, env)
            else:
        ...
        # the difference between lambda and mu procedure.
